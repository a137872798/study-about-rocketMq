...
public class PullMessageProcessor implements NettyRequestProcessor {
    ...
    private RemotingCommand processRequest(final Channel channel, RemotingCommand request, boolean brokerAllowSuspend)
        ...
        //如果 不可读 就没有权限  拉取消息 就是从 broker 获取消息
        ...
        //看看 拉取的 是什么消息  这里是用 组来确定订阅信息在哪的
        //传来的参数会 提示是哪个消费者组的 然后可以获取到订阅的 消息
        ...
        //队列id 如果 大于 可读数  就是异常  因为队列id 好像就是 队列数-1 然后 可读的 必然<= 总数
        ...
        if (hasSubscriptionFlag) {
            ...
            //订阅数据版本对不上 因为其他消费者 在心跳检测时 将 订阅消息 更新了
            ...
            //如果是 sql92类型就创建 消费者拦截对象
            if (!ExpressionType.isTagType(subscriptionData.getExpressionType())) {
                //直接从缓存中获取
                consumerFilterData = this.brokerController.getConsumerFilterManager().get(requestHeader.getTopic(),
                    requestHeader.getConsumerGroup());
                //拿不到直接异常 这个对象在 brokerController 对象创建时 就初始化了
                if (consumerFilterData == null) {
                    response.setCode(ResponseCode.FILTER_DATA_NOT_EXIST);
                    response.setRemark("The broker's consumer filter data is not exist!Your expression may be wrong!");
                    return response;
                }
                //版本低  看来每个消费者 也有可能会更新这个对象
        ...
        //根据请求要求的偏移量去 messagestore 中获取对象的数据
        //传入的偏移量是 cq 的 然后 里面可以拿到 commitLog 的偏移量
        //同时这里进行了 2次过滤
        ...
        if (getMessageResult != null) {
            ...
            switch (response.getCode()) {
                ...
                case ResponseCode.PULL_NOT_FOUND:
                    ...
                    //数据没有找到时 有 suspend 标识时 目前在 push中 有设置这个标识  pull 没有
